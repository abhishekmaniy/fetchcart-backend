...
async function sendToGemini(prompt: string) {
  ...
  if (match) {
    text = match[0] // keep only the JSON object
  }

  try {
    return JSON.parse(text)
  } catch (err) {
    console.error('Gemini Output after cleanup:', text)
...
const search = async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.log("Shopping Resulted Founded by serpapi...")
    ...
    for (const url of productLinks) {
      try {
        ...
        // this for structured and detailed data scraping for particular product 
        ...
        const scraperRes = await axios.get(
          `https://api.scraperapi.com/?api_key=${SCRAPER_API}&url=${productLinks[0]}&output_format=text&device_type=desktop&country_code=in`
        );

        const Product_Url_Getter_Prompt = `
        You are given raw scraped data from a Google Shopping page.
        
        Your task:
        - Extract the main product URL (the real store/product page link, e.g. from amazon.in, flipkart.com, asus.com, etc.).
        ...
        const geminiResponse = await sendToGemini(Product_Url_Getter_Prompt)
        console.log("Product Url Generated by gemini model...", geminiResponse.productUrl)

        const encodedProductUrl = encodeURIComponent(geminiResponse.productUrl);

        const scrapeRes = await axios.get(
          `https://api.scraperapi.com/?api_key=${SCRAPER_API}&url=${encodedProductUrl}&output_format=json&autoparse=true&device_type=desktop&country_code=in`
        );

        const scrapedProduct = scrapeRes.data
        scrapedProduct.productUrl = encodedProductUrl
